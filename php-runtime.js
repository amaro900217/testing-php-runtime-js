...
window.php = {
  ...
  _wasmBufferPromise: null,
  _queue: [],
  _nextId: 0,

  _log(...args) { if (this._config.DEBUG) { console.log('[PHP Runtime]', ...args); } },
      ...
      this._wasmBufferPromise = (async () => {
        const db = await new Promise((resolve, reject) => {
          ...
          request.onupgradeneeded = e => {
            ...
            if (!db.objectStoreNames.contains('FILE_DATA')) db.createObjectStore('FILE_DATA');
          };
        ...
        if (!response.ok) throw new Error(`❌ Failed to download WASM: ${response.status}`);
  ...
  async _installPhpFiles(wasmBuffer) {
    let phpWeb = new PhpWeb({ wasmBinary: wasmBuffer, persist: { mountPath: "/www" } });
    await phpWeb.ready;
    ...
    if (!response.ok) throw new Error(`❌ Failed to download php.zip: ${response.statusText}`);
  ...
  async _spawnWorkers(NUM_WORKERS, wasmBuffer, config) {
    for (let i = 0; i < NUM_WORKERS; i++) {
      const worker = new Worker(new URL('./php-worker.js', import.meta.url), { type: 'module' });
      worker.available = false;
      worker.onmessage = (e) => this._handleWorkerMessage(worker, e);
      this.workers.push(worker);
      worker.postMessage({ type: 'loadWasm', wasmBin: wasmBuffer, cnfg: config });
    }
  },
  ...
  _processQueue() {
    for (const worker of this.workers.filter(w => w.available)) {
      ...
      task.assigned = true;
      worker.available = false;
      worker.postMessage({ type: task.type, id: task.id, request: task.request });
    }
  },
